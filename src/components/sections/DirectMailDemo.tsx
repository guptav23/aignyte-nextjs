    ...
    <div className={styles.visualArea}>
      ...
      <div className={styles.decodedPanel}>
        <div className={styles.decodedHeader}>
          <div className={styles.decodedTitle}>Resulting Genetic Signature</div>
          <div className={styles.decodedSubtitle}>4 dimensions, &apos;000s of data points — capturing everything about the creative at an atomic level</div>
          <div className={styles.decodedNote}>
            <span className={styles.decodedNoteIcon}>ℹ</span>
            <span>Illustrative sample — a real Genetic Signature contains &apos;000s of elements across these 4 dimensions</span>
          </div>
        </div>

        {/* A — Creative Elements */}
        <div className={styles.decodedGroup}>
          <div className={styles.decodedGroupLabel}>A — Creative Elements</div>
          <div className={styles.decodedItems}>
            {['Dominant Color: Blue', 'Card Imagery', 'Envelope Message'].map((name) => (
              <div key={name} className={styles.decodedItem}>
                <div className={styles.decodedItemName}>{name}</div>
                <div style={{ flex: 1 }} />
                <div className={styles.presentBadge}>Present</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* B — Emotions */}
        <div className={styles.decodedGroup}>
          <div className={styles.decodedGroupLabel}>B — Emotions</div>
          <div className={styles.decodedItems}>
            {[{ name: 'Optimism', pct: 55, val: '1.1' }, { name: 'Aspiration', pct: 45, val: '0.9' }].map((item) => (
              <div key={item.name} className={styles.decodedItem}>
                <div className={styles.decodedItemName}>{item.name}</div>
                <div className={styles.decodedBar}><div className={styles.decodedFill} style={{ width: `${item.pct}%` }} /></div>
                <div className={styles.decodedVal}>{item.val}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className={styles.decodedGroup}>
          <div className={styles.decodedGroupLabel}>C — Value Propositions</div>
          <div className={styles.decodedItems}>
            {[
              { name: 'Miles Rewards', pct: 86, val: '8.6' },
              { name: 'CTA: Online', pct: 70, val: '7.0' },
              { name: 'Ease of Application', pct: 42, val: '4.2' },
            ].map((item) => (
              <div key={item.name} className={styles.decodedItem}>
                <div className={styles.decodedItemName}>{item.name}</div>
                <div className={styles.decodedBar}><div className={styles.decodedFill} style={{ width: `${item.pct}%` }} /></div>
        ...
        {/* D — Product Terms */}
        <div className={styles.decodedGroup}>
          <div className={styles.decodedGroupLabel}>D — Product Terms</div>
          <div className={styles.decodedItems}>
            {[
              { name: 'Bonus Miles: 60,000', pct: 100, val: '11.0' },
              { name: 'Fast Approval', pct: 82, val: '8.2' },
              { name: 'No Annual Fee', pct: 50, val: '5.0' },
              { name: 'Pre-screened Offer', pct: 68, val: '6.8' },
            ].map((item) => (
              <div key={item.name} className={styles.decodedItem}>
                <div className={styles.decodedItemName}>{item.name}</div>
                <div className={styles.decodedBar}><div className={styles.decodedFill} style={{ width: `${item.pct}%` }} /></div>
